...
- [x] F005 TDD: 必殺技制約チェック ✅
  ...
  - ✅ 4つのテスト全通過
- [x] F006 TDD: キャラクター作成フォームUI実装（基本版） ✅
  - ✅ CharacterFormコンポーネント実装（FSDアーキテクチャ準拠）
  - ✅ フォームバリデーション・エラー表示機能
  - ✅ packages/core統合（calculateAbilities関数使用）
  - ✅ vitest + React Testing Library環境構築
  - ✅ 10のテストケース全通過
  - ⚠️ **spec.md要件の40%のみ実装済み（受け入れシナリオ1-1, 1-2のみ）**

**🚨 緊急対応が必要な未実装機能（spec.md要件）**:
- [ ] F006-2 TDD: 技能ポイント割り振りUI実装（FR-016, シナリオ1-3）
- [ ] F006-3 TDD: ヒーロースキル選択UI実装（FR-018, シナリオ1-4）
- [ ] F006-4 TDD: 必殺技選択UI実装（FR-019, シナリオ1-4）
- [ ] F006-5 TDD: アイテム購入UI実装（FR-020, シナリオ1-4）
- [ ] F006-6 TDD: HP・SP・行動値算出・表示UI実装（FR-021, FR-022, シナリオ1-5）
- [ ] F006-7 TDD: マルチステップフォーム実装（全体統合）
- [ ] F006-8 TDD: キャラクター保存・URL発行機能実装（FR-006, シナリオ3）
- [ ] F006-9 TDD: パスワード保護機能実装（FR-023, シナリオ3）

- [ ] F007 TDD: セッション管理UI実装（FR-024, シナリオ6）

...
**作成日**: 2025-09-08  
**ステータス**: 設計更新済み（データモデル簡素化・セッション履歴管理対応）  
...
3. 不明確な側面について:
   ...
   → 解決済み: URLベースのアクセス制御（ユーザー認証なし、オプションでパスワード保護）
   → 解決済み: データは永続保存、削除は行わない仕様（10人未満の小規模サービス）
   → 追加解決: データベース設計（正規化 → 単一テーブル＋JSON設計へ変更）
   → 追加解決: セッション履歴管理（キャラクター基本情報から分離し、配列形式で管理）
   → 追加解決: バリデーション方針（厳密性 → ユーザビリティ重視の寛容な検証へ）
...
3. **前提** スキル・装備選択完了, **実行** 保存ボタンを押下, **結果** キャラクター情報がオンラインに保存され、一意のURLが発行される（オプションでパスワード設定可能）
4. **前提** キャラクターシートURLを知っているユーザーがアクセス, **実行** HP・SP・装備の変更を入力, **結果** リアルタイムで変更が保存され、他のデバイスからも同期される（パスワード設定時はパスワード入力が必要）
6. **前提** セッション実施時, **実行** セッション記録の追加・編集, **結果** セッション名、GM名、実施日、現在HP・SP・FC値、獲得経験点、メモを含むセッション履歴が追加・更新される
...
- **FR-006**: システムはURLベースのアクセス制御を提供しなければならない（ユーザー認証なし、URLを知っていれば編集可能）
- **FR-023**: システムはオプションでパスワード保護機能を提供しなければならない（パスワード設定時はパスワード入力が必要）
- **FR-007**: システムはキャラクターデータを永続的に保持しなければならない（削除機能は提供せず、永続保存仕様）
...
- **FR-018**: システムはヒーロースキル取得機能を提供しなければならない（合計7Lv分を任意のヒーロースキルに割り振り、スキルの完全な情報セット（名前、レベル、タイミング、効果等）を保存）
- **FR-019**: システムは必殺技取得機能を提供しなければならない（1Lvの必殺技を取得、必殺技の完全な情報セット（名前、レベル、タイミング、効果等）を保存）
...
- **FR-024**: システムはセッション履歴管理機能を提供しなければならない（セッション名、GM名、実施日、現在HP・SP・FC、獲得経験点、メモを記録）
- **FR-025**: システムは単一データベーステーブル＋JSONカラム設計で実装しなければならない（シンプルさとパフォーマンスを重視）
- **FR-026**: システムはデータ入力時の制約を最小限に抑えなければならない（ユーザビリティを重視し、空文字や不完全な入力も受け入れる）
...
- **Character**: Age of Heroキャラクター情報
  ...
  - ヒーロースキル取得: 合計7Lv分を任意のヒーロースキルに割り振り（完全な情報セット保存）
  - 必殺技取得: 1Lvの必殺技を取得（完全な情報セット保存）
  ...
  - 基本ステータス: HP・SP・FC等のベース値
  - 行動値: 反射×2+知力で算出、ヒーロースキル修正含む
  - セッション履歴: セッションごとの現在値（HP・SP・FC）、経験点、メモを配列で管理
- **HeroSkill**: ヒーロースキル詳細
...
- **CharacterAccess**: キャラクターアクセス制御
  - URL: 一意のアクセスURL
  - パスワード: オプションのパスワード保護
  - 作成日時: キャラクター作成タイムスタンプ
- **SessionHistory**: セッション履歴情報
  - セッション記録: セッション名、GM名、実施日、現在HP・SP・FC、獲得経験点、メモ
  - 配列形式: 時系列でのセッション履歴を管理
  - 最小限バリデーション: 空文字や不完全データも受け入れ可能
...
### 要件完全性
- [x] [要明確化] マーカーが残っていない - **全て解決済み**
- [x] 要件がテスト可能で曖昧でない  
- [x] 成功基準が測定可能
- [x] スコープが明確に境界設定
- [x] 依存関係と前提条件が特定済み
- [x] データモデル設計方針が確定 - **シンプルさ優先、単一テーブル＋JSON**
- [x] セッション履歴管理要件が追加 - **配列形式での時系列管理**
- [x] バリデーション要件が緩和 - **ユーザビリティ重視**
...
- [x] 要件を生成
- [x] エンティティを特定
- [x] レビューチェックリスト通過 - **全ての明確化が完了**
- [x] データモデル簡素化対応 - **単一テーブル＋JSON設計へ変更**
- [x] セッション履歴管理追加 - **配列形式での履歴管理機能**
- [x] バリデーション方針更新 - **ユーザビリティ重視の寛容な検証**

## 設計変更履歴
*データモデル簡素化・セッション履歴管理対応で追加*

### 主要な設計判断
1. **データベース設計の簡素化**
   - 変更前: 正規化された複数テーブル構成
   - 変更後: 単一テーブル＋JSONカラム設計
   - 理由: 小規模サービス(<100キャラクター)でのシンプルさとパフォーマンス重視

2. **セッション履歴の分離管理**
   - 変更前: キャラクター基本情報内でcurrentHP/SP/FC管理
   - 変更後: セッション履歴として配列形式で独立管理
   - 理由: セッションごとの状態変化を時系列で追跡可能にするため

3. **バリデーション方針の緩和**
   - 変更前: 厳密なスキーマ検証
   - 変更後: 最小限の制約、空文字・不完全入力も許容
   - 理由: TRPG用途でのユーザビリティを最優先

4. **共通スキーマの抽出**
   - ClassNameSchema: 8つのクラス名の共通enum
   - AbilityValueSchema: 能力値の共通バリデーション
   - 理由: DRY原則によるコード重複削減

### 影響範囲
- フロントエンド: API インターフェース変更に伴うコンポーネント調整
- バックエンド: データベーススキーマとORMマッピングの全面変更
- テスト: データモデル変更に伴うテストケース更新

...
export const TechnologyClassPage: React.FC = () => {
  const classData: ClassPageData = {
    ...
    originsDescription: 'テクノロジーヒーローの装備がどのようにして得られたかは様々である。キャラクター作成時に、自分のテクノロジーヒーローがどのような経緯で装備を得たのかを考えてみよう。',
    originsNote: 'いずれの起源であっても、それらの装備は一様に強力で、持ち主であるヒーローのみが扱える特別な存在なのだ。',
    abilityNote: 'テクノロジーは感覚能力（3）に特化し、知力・反射も平均的な値を持つ技術特化型クラス。肉体は低く、超常能力は持たないが、アイテムによって多様な能力を発揮できる。',
    playStyles: [
      {
        title: '戦闘での役割',
        color: 'bg-green-50',
